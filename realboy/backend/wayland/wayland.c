...
void backend_wayland_update_frame() {
	struct wl_buffer *wl_buf = wl_shm_pool_create_buffer(shm_pool, 0, WIDTH, HEIGHT, STRIDE, WL_SHM_FORMAT_XRGB8888);
...
void backend_wayland_init() {
	...
	shm_pool = wl_shm_create_pool(shm, fd, stride * height);
