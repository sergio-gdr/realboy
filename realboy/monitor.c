	...
	if (wayland_backend_is_focus()) {
		switch (ev->code) {
			...
			case KEY_L:
				...
				break;
			default:
				perror("monitor_set_key");
		}
		// request interrupt on key press.
		if (ev->value) {
			cpu_request_intr(REQUEST_INTR_JOYPAD);
		}
	}
}
...
static void *wait_for_server_request(void *val) {
	...
	pthread_cleanup_push(thread_cleanup, _);
...
static void create_wait_thread(pthread_t *epoll_thread) {
	int pipefds[2];
	if (pipe(pipefds) == -1)
		perror("pipe()");

	if (pthread_create(epoll_thread, NULL, wait_for_server_request, &pipefds[1]))
		perror("pthread_create()");

	int _;
	if (read(pipefds[0], &_, 1) == -1)
		perror("read()");
	...
	while (1) {
		...
		if (server_is_client_connected()) {
			if (monitor_is_executing) {
				create_wait_thread(&poll_thread);
				do {
					exec_next();
				} while (should_continue_execution());
				if (pthread_cancel(poll_thread) != 0)
