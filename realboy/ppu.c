...
static void ppu_render_line() {
	...
	//printf("wx %d wy %d\n", regs->wx, regs->wy);
	win_tilemap = win_tilemap_row_beg;
	uint8_t win_beg_pixel_in_tile = regs->wx&7;
	for (int i=0; i < SCANLINE; i+=8) {
		for (int j = 0; j < 8; j++) {
			...
			if (regs->lcdc & LCDC_BITMASK_OBJ_ENABLE) {
				...
				if (num_objs) {
					...
					if (obj_indx) {
						...
						obj_pixel = color_pal[(obj_palette & (3<<(obj_indx*2)))>> (2*obj_indx)];
			...
			if (regs->lcdc & LCDC_BITMASK_BG_ENABLE &&
				...
				assert(bg_indx <= 3);
				uint32_t bg_pixel = color_pal[(regs->bgp & (3<<(bg_indx*2))) >> (2*bg_indx)];
				if (!obj_prio || (bg_pixel != 0))
					final_pixel = bg_pixel;
			}

			if (regs->lcdc & LCDC_BITMASK_WIN_ENABLE &&
					(!obj_pixel || obj_prio[3] & 0x80) &&
					(regs->wy <= regs->ly && regs->wx < 166 && regs->wy < 143)) {
				// adjust the column position each iteration, since it may wrap at any time
				// in the line rendering.
				win_tilemap = win_tilemap_row_beg + (((i + j + (regs->wx)))>>3);

				// get bg tileline
				uint16_t *win_tileline;
				win_tileline = get_tile_line(*win_tilemap, (regs->ly-regs->wy)&7);

				uint8_t win_indx = (bool)(win_tileline[0] & (0x8000 >> ((j+win_beg_pixel_in_tile)&7)))<<1;
				win_indx |= (((bool)(win_tileline[0] & (0x80 >> ((j+win_beg_pixel_in_tile)&7)))));
				assert(win_indx <= 3);

				uint32_t win_pixel = color_pal[(regs->bgp & (3<<(win_indx*2)))>> (2*win_indx)];
				if (!obj_prio || (win_pixel != 0))
...
static void change_phase() {
	switch (ppu.mode) {
		...
		case PPU_DRAW:
			...
			break;
		default:
			printf("change_phase(): ppu_mode %x\n", ppu.mode);
	}
}
...
#include "cpu/cpu.h" // XXX wr and rd are private to the cpu module
					 // replace with a public cpu_do_dma()
